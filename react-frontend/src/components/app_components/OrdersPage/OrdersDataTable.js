...
const multiselectTemplate1 = (rowData, { rowIndex }) => (<ul className="list-none" >{rowData.products.map((i,x) => (<li key={x}> name: {i.name},sku: {i.sku}</li>))}</ul>)
...
<Column field="products" header="Products" body={multiselectTemplate1} filter={selectedFilterFields.includes("products")} hidden={selectedHideFields?.includes("products")}  style={{ minWidth: "8rem" }} />
